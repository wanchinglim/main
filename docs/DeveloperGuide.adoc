...
1. *Subjects Panel* (facilitated by `SubjectListPanel`): +
Displays:
** Subject tag

2. *Flashcards Panel* (facilitated by `FlashcardListPanel`): +
...
3. *Content Panel* (facilitated by `RightPanel`): +
This panel is further sectioned into 2:
** Topic of flashcard
** Content of flashcard

image::UI copy.png[width=800]
...
the subject tag "Math" will be added to a `SubjectBook` object and `UniqueSubjectList` object before being displayed in the `SubjectListPanel`. +
...
* The UI component's `MainWindow` class will create a new `SubjectListPanel` object that takes in and invokes:
...
* In the `SubjectListPanel` class,
** The functions `setItems()` and `setCellFactory()` of JavaFX will populate the panel with the unique subject list from `getFilteredSubjectList()`.

* The UI component's `MainWindow` class will create a new `FlashcardListPanel` object that takes in and invokes:

** Logic component's `getFilteredFlashcardList()` -- returns `filteredFlashcards` which contains the list of unique flashcards
** Logic component's `selectedSubjectProperty()` -- returns the `selectedSubject` tag clicked by the user

* In the `FlashcardListPanel` class,
** The functions `setItems()` and `setCellFactory()` of JavaFX will populate the panel with the unique flashcard list from `getFilteredFlashcardList()`.
