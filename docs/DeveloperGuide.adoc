...
** Subject displayed as a colour-coded tag
...
Upon adding a flashcard with the `add` command, (e.g. `add t/Pythagoras Theorem diff/1 c/a^2 + b^2 = c^2 d/30-03-2019 s/Math`),
the subject tag "Math" will be added to a `UniqueSubjectList` and subsequently displayed in the `SubjectListPanel`. +

An example scenario of adding a flashcard and displaying the flashcards for each subject is shown below.
...
Step 1 -- Each time a flashcard is added using the Logic component's `AddCommand()`, the adding will be facilitated by `addSubject()` in the Model component.

Step 2 -- The Model component's `addSubject()` will use `SubjectBook` 's `addSubject()` to add the subject to a list containing only unique subjects.

Step 3 -- `SubjectBook` will create a `UniqueSubjectList` object that stores all unique subjects.

Step 4 -- The `UniqueSubjectList` object will invoke its `addSubject()` function. This function will check if the current `UniqueSubjectList` object contains the subject tag of the current flashcard to be added.
...
Step 5 -- The `SubjectListPanel` then displays this newly added subject.

+
The following sequence diagram depicts what happens in this scenario: +
<< SEQUENCE DIAGRAM >>
...
Step 5 -- The `SubjectListPanel` remains unchanged.
...
Step 1 -- The UI component's `MainWindow` will create a new `SubjectListPanel` object that takes in and invokes:
...
Step 2 -- In `SubjectListPanel`, `setItems()` and `setCellFactory()` of JavaFX will populate the panel with the unique subject list from `getFilteredSubjectList()`.

Step 3 -- The UI component's `MainWindow` will create a new `FlashcardListPanel` object that takes in and invokes:

* Logic component's `getFilteredFlashcardList()` -- returns `filteredFlashcards` which contains a list of unique flashcards
* Logic component's `selectedSubjectProperty()` -- returns the `selectedSubject` tag clicked by the user

Step 4 -- In `FlashcardListPanel`, `setItems()` and `setCellFactory()` of JavaFX will populate the panel with the unique flashcard list from `getFilteredFlashcardList()`.

Step 5 -- A listener is attached to the `selectedSubjectProperty` value passed into `FlashcardListPanel` in order to detect if another subject is selected.

Step 6 -- When a new subject is selected, `FlashcardListPanel` invokes `updateFlashcardList()`.
...
Step 7 -- The flashcard panel is then populated with flashcards from `newFlashcardList` using `setItems()` and `setCellFactory()` of JavaFX.
