...
* `Flashcards#next()` -- Opens the next flash card in line.
* `Flashcards#previous()` --  Restore the previous flash card.
...
1) `"Subject" Testing Mode` +
2) `"Focus" Testing Mode`

==== `"Subject" Testing Mode`

This mode will be largely focused on viewing flash cards based on a particular subject. From the pool of flash cards, the user can choose to select all flash cards that are under the same subject. This mode will focus on a specific subject and can be used for revision before mini quizzes of that particular subject.
