...
[NOTE]
As a more OOP model, we can store a `Tag` list in `Flash Book`, which `Flashcard` can reference. This would allow `Flash Book` to only require one `Tag` object per unique `Tag`, instead of each `Flashcard` needing their own `Tag` object. An example of how such a model may look like is given below. +
 +
...
It ia stored internally and implements the following operations:
...
* `FlashCards#c()` -- (change) Closes the flash cards in the current flashcard and move on to the next flashcard selected/in line.
...
The Subject tag will be added to the `UniqueSubjectList`.
+
...
No Subject tag is added to the `UniqueSubjectList`.
+
...
[NOTE]
If a command fails its execution, it will not call `Model#commitAddressBook()`, so the flash book state will not be saved into the `addressBookStateList`.
...
** Cons: Might be too troublesome for some users.
...
* **Alternative 2:** Use the `clear` or `clearall` functions.
** Pros: Give the user the choice to keep or delete.
** Cons: Troublesome for users.
// end::undoredo[]
...
*Scenario:* You are in charge of `storage`. For your next project milestone, your team plans to implement a new feature of saving the flash book to the cloud. However, the current implementation of the application constantly saves the address book after the execution of each command, which is not ideal if the user is working on limited internet connection. Your team decided that the application should instead save the changes to a temporary local backup file first, and only upload to the cloud after the user closes the application. Your job is to implement a backup API for the flash book storage.
...
|`* * *` |New User |Create a new account |Only see flash cards and flashcards that are related and customised for me
...
|`* * *` |User |List flash cards categorized by flashcards |Have an overview of the flash cards I currently have
...
1.  FlashCards displays an indexed list of flash cards categorized by flashcards
+
Use case ends.
